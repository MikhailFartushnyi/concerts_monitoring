...
Если у события есть discountPercent — дополнительно запрашиваем promo_cards
...
HEADERS = {
    ...
    'pragma': 'no-cache',
    'priority': 'u=1, i',
    'referer': 'https://afisha.yandex.ru/moscow/concert/kinomuzyka-pri-svechah?source=selection-events&selectionPage=discount-classical-concerts&refererSelection=main_page_landing&refererSurface=selection',
...
def run() -> list[dict]:
    ...
    with tqdm(desc="  afisha.yandex", unit=" концерт", leave=True) as pbar:
        while True:
            json_data = {
...
def _parse(session: requests.Session, items: list[dict]) -> list[dict]:
    ...
    for d in items:
        ...
        # Дата — текстовое превью расписания
        date_txt = (d.get("scheduleInfo", {}).get("preview") or {}).get("text", "") or ""

        # Цены (в копейках → рублях)
        ...
        # Запрашиваем promo_cards только если есть скидка
        if not discount_pct and event_id:
            promo_code = _fetch_promo_code(session, event_id)
            time.sleep(0.3)
...
def _fetch_promo_code(session: requests.Session, event_id: str) -> str:
    """Запрашивает promo_cards и извлекает промокод из nameHtml вида <b>КОД</b>."""
    try:
        r = requests.get(
    ...
    for card in cards.values():
        ...
        # Ищем промокод в <b>КОД</b>
        m = re.search(r"<b>([A-Za-zА-Яа-я0-9_\-]+)</b>", name_html)
        if m:
            return m.group(1)
    return ""
